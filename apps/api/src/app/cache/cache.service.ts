...
export class CacheService {
  ...
  private readonly KEYS = {
    ...
    QUESTION_BANK_DETAIL: (userId: string, qbId: string) =>
      `qb:detail:${userId}:${qbId}`,
    QUESTION_BANK_STATS: (userId: string, qbId: string) =>
  ...
  async invalidateQuestionBankDetail(
    userId: string,
    qbId: string
  ): Promise<void> {
    const key = this.KEYS.QUESTION_BANK_DETAIL(userId, qbId);
  ...
  async invalidateQuestionBankStats(
    userId: string,
    qbId: string
  ): Promise<void> {
    const key = this.KEYS.QUESTION_BANK_STATS(userId, qbId);
