...
export class QuestionImportComponent {
  ...
  public questions$ = this.control.valueChanges.pipe(
    ...
    map(() => {
      ...
      const questions = this.control.value
        ?.replace(/\n/g, ' ')
        .split(/(?=\b\d{1,2}\b\.)/);
      this.parsedQuestions = questions?.map(
  ...
  async import() {
    ...
    if (!dto || dto.length === 0) {
      this.snackbar.open('No valid questions to import', 'Close', {
        duration: 3000,
      });
      return;
    }
    ...
    try {
      ...
      this.control.reset('');
      this.snackbar.open(
        `Successfully imported ${dto.length} questions`,
        'Close',
        { duration: 3000 }
      );
    } catch (error) {
